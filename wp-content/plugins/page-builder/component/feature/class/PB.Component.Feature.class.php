...
class PBComponentFeature extends PBComponent
{
	...
	function processShortcodeFeature($attribute,$content,$tag)
	{
		...
		$this->layoutAttr=$attribute['layout'];
		$this->layoutAttrColumnIndex=0;
	...
	function processShortcodeFeatureItem($attribute,$content,$tag)
	{
		...
		(
			array
			(
				'pb-layout-'.$Layout->getLayoutColumnCSSClass($this->layoutAttr,$this->layoutAttrColumnIndex),
				'pb-feature-element-'.$position
			),
			array
			(
				'pb-feature-icon',
				'pb-feature-icon-'.PBHelper::createHash($attribute['item_icon'])
			)
		);
